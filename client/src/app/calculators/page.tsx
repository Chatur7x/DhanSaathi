      ...
      <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
        <motion.div variants={item}>
          <h1 className="text-2xl font-extrabold text-foreground flex items-center gap-3">
            <Calculator size={28} className="text-primary" /> Financial Calculators
          </h1>
          <p className="text-sm text-muted-foreground mt-1">Precision tools for smart financial planning</p>
        </motion.div>
            ...
            <motion.div key={calc.name} variants={item}>
              <Link href={calc.href}>
                <GlowCard glowColor={calc.color} className="group cursor-pointer h-full">
                  ...
                  <h3 className="text-sm font-bold text-foreground mb-1 group-hover:text-indigo-300 transition-colors">{calc.name}</h3>
