    ...
    <AppShell>
      <motion.div initial="hidden" animate="show" variants={{ hidden: { opacity: 0 }, show: { opacity: 1, transition: { staggerChildren: 0.07 } } }} className="space-y-6">
        <motion.div variants={item} className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-foreground">SIP Calculator</h1>
            <p className="text-sm text-muted-foreground">Systematic Investment Plan Growth Projections</p>
          </div>
        </motion.div>
          ...
          <motion.div variants={item}>
            <GlowCard glowColor="#10b981" className="space-y-6">
              {[
                ...
                <div key={s.label}>
                  <div className="flex justify-between mb-2">
                    <label className="text-xs text-muted-foreground font-semibold uppercase tracking-wider">{s.label}</label>
                    <span className="text-sm font-bold text-primary">{s.prefix || ""}{s.value.toLocaleString("en-IN")}{s.suffix || ""}</span>
                  </div>
              ...
              <div className="grid grid-cols-3 gap-3 pt-2">
                {[
                  { label: "Invested", value: result.invested, color: "text-foreground" },
                  { label: "Wealth Gained", value: result.wealth, color: "text-emerald-600 dark:text-emerald-400" },
                  { label: "Total Value", value: result.futureValue, color: "text-primary" },
                ].map(r => (
                  <div key={r.label} className="text-center p-3 rounded-xl bg-accent/50 border border-border">
                    <p className="text-[0.6rem] text-muted-foreground uppercase tracking-widest font-semibold">{r.label}</p>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={result.chartData}>
                    ...
                    <Tooltip contentStyle={{ background: "#0f172a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px", color: "#fff" }}
                      formatter={(v: unknown) => [`₹${Number(v ?? 0).toLocaleString("en-IN")}`, ""]} />
